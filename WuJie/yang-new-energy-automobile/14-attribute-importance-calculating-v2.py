...
def produce_corpus(origin_data):
    ...
    for current_type in types:
        current_data = origin_data.loc[origin_data["来源类型"] == current_type]
        years = set(current_data["购买年份"].tolist())
        for year in years:
            current_data_year = current_data.loc[current_data["购买年份"] == year]
            # 处理每个属性
            for aspect in cols:
                print(current_type, year, aspect)
                if aspect in ["购买年份", "来源类型"]:
                    continue
...
if __name__ == "__main__":
    ...
    data = pd.read_excel(path, usecols=[9, 10, 13, 17, 19, 21, 23, 25, 27, 29, 31], engine='openpyxl')  # 分属性评论的语料库
    print(data.columns)
    ...
    # 计算tfidf
    print("开始计算tfidf")
    s_path = "test/hot_words-v2-test.csv" if debug else "result/attribute-importance-来源类型.csv"  # 分属性评论的语料库
