...
def load_data(path, s_path):
    ...
    # 提取短文本
    print("正在提取短文本")
    current_data["shortTexts"] = current_data.apply(lambda row: get_abstract(row['评价内容']), axis=1)
...
if __name__ == "__main__":
    ...
    s_path_global = "test/shortTexts-test.xlsx" if debug else "result/shortTexts.xlsx"
    # 判断文件是否存在，如果不存在则执行
    ...
    # 短文本情感计算
    s_path_score_global = "test/merged-score-test.xlsx" if debug else "result/merged-score.xlsx"
    # 判断文件是否存在，如果不存在则执行
