...
def Parser2dataframe(words,postags,arcs):
    ...
    tuples_words = pd.DataFrame({'word':list(word_dict.values()),'word_n':list(word_dict.keys()),                             'match_word':match_word,'relation':relation,'pos':pos,'match_word_n' : match_word_n})
    tuples_words['tuples_words'] = get_tuples_word(tuples_words['word'],tuples_words['word_n'],                                                   tuples_words['match_word'],tuples_words['match_word_n'])
    return tuples_words
...
'''
#  模型文件
ltp = ltp_api(r"C:\Softwares\coding\Ltp\ltp_data")
# ltp.release()
print("模型加载完毕")
...
data=pd.read_excel('data/test/观点抽取数据-Strength&Opportunity.xlsx', engine="openpyxl")

print(len(data))
content=['Location']
# content=['Location','Service', 'Parking', 'Wifi', 'Transport', 'Surroundings']


for con in content:
    for i in range(len(data)):
        if data[con][i]!=0:
            paragraph = data[con][i]
            sentences = SentenceSplitter.split(paragraph)
            for sentence in sentences:
                if sentence:
                    # 第一种：类内计算
                    ...
                    labeller = ltp.ltp_labeller(words,postags, arcs) #语义角色
                    tuples_words = Parser2dataframe(words,postags,arcs)
            data[con][i]="".join(CoreExtraction(tuples_words))


print("正在保存哦")
data.to_excel('data/test/观点抽取数据-Strength&Opportunity-Service-done.xlsx')
