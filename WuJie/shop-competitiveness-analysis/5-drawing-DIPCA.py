...
def drawing_2D_3D_3(path, name):
    ...
    # quadrant = "121"
    # ax = figure.add_subplot(quadrant, projection='3d')
    ax1 = figure.add_axes([-0.12, 0.15, 0.65, 0.7], projection='3d')
    ...
    for aspect in aspects:
        ...
        current_color = "#" + str(current_data_aspect["Color"].tolist()[0])
        print("current_color:", current_color)
        ax1.plot(x, y, z, linewidth=2.5, color=current_color, linestyle='-', label=aspect)  # 设置图例
        # 3D散点图
        length = len(y)
        for j in range(length):
            current_x = x[j]
    ...
    ax1.set_xlabel("Year", fontdict=font2, labelpad=15.5)
    ax1.set_ylabel("PD", fontdict=font2, labelpad=15.5)
    ax1.set_zlabel("GAP", fontdict=font2, labelpad=20.5, rotation=90)  # 设置轴标签与轴的距离
    ...
    ax1.set_xticklabels(["2019", "2020", "2021"], fontdict=font)
    ...
    # quadrant = "122"
    # ax = figure.add_subplot(quadrant)
    ax2 = figure.add_axes([0.51, 0.18, 0.33, 0.62])  # 无背景色
    # ax2 = figure.add_axes([0.52, 0.17, 0.35, 0.67], facecolor='#f2f2f2')
    # 设置边框颜色和宽度
    linewidth = 2
    ...
    plt.rcParams['font.sans-serif'] = ['Simhei']
    plt.rcParams['axes.unicode_minus'] = False
    leg = plt.legend(aspects, loc="lower center", bbox_to_anchor=(1.25, -0.02), prop=font3, borderaxespad=0., markerscale=5)  # 设置图例
    leg.get_frame().set_linewidth(0.0)  # 去掉图例边框
...
if __name__ == "__main__":
    ...
    path_global = "data/属性情感-汇总后-v3（19-21）.xlsx"
    table_name = "drawing"

    drawing_2D_3D_3(path_global, table_name)
