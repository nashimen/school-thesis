...
def searchComponentTfidf():
    df = pd.DataFrame(columns=('component', 'opinion', 'tfidf'))
    # 检索哦
    for component in field_words_dict.keys():
        opinions = field_words_dict.get(component)
        for opinion in opinions:
            t = wordTfidf.get("key").get(opinion, 'no_value')
            df = df.append([{'component': component,  'opinion': opinion, 'tfidf': t}], ignore_index=True)
    return df


# 检索分属性部件的TFIDF值，并保存至文件
print("正在处理" + "satisfy" if satisfy else "unsatisfy" + "_content...")
keyWords = transMatrix(satisfy_content.values() if satisfy else unsatisfy_content.values())
result = searchComponentTfidf()
print(result.head())
# 保存至文件
result_path = "result/" + file_name + "_" + str("satisfy" if satisfy else "unsatisfy") + ".csv"
